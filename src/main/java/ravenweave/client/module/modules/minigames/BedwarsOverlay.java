...
public class BedwarsOverlay extends Module {
    ...
    public BedwarsOverlay() {
        ...
        this.registerSetting(bombiesMomento = new DescriptionSetting("Stats overlay for bedwars"));
        overlayHeight = 170;
        overlayWidth = 300;
    ...
    @Subscribe
    public void onForgeEvent(ChatReceivedEvent e) {
            if (Utils.Player.isPlayerInGame()) {
                if (Utils.Java.str((e).getMessage().getUnformattedText())
    ...
    private void drawStats(NetworkPlayerInfo player, FontRenderer fr) {
        ...
        if (Utils.URLS.hypixelApiKey.isEmpty()) {
            ...
            bbblr = stats[4] != 0 ? Utils.Java.round((double) stats[3] / (double) stats[4], 2) : stats[3];
            fkdr = stats[6] != 0 ? Utils.Java.round((double) stats[5] / (double) stats[6], 2) : stats[5];
            wlr = stats[8] != 0 ? Utils.Java.round((double) stats[7] / (double) stats[8], 2) : stats[7];
